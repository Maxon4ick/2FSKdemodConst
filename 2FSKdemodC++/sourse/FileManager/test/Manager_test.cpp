...
TEST(FileTests,Convert)
{
    const std::string fileNameData = "Test1.txt";
    const std::string fileNameTest = "Test2.txt";
    std::vector<float> test = FileManager::readBin<float>(fileNameTest);
    std::vector<short> ex = FileManager::readBin<short>(fileNameData);
    std::vector<float> out =FileManager::convertSignal(ex);
    EQUAL_VEC(out,test);
}
